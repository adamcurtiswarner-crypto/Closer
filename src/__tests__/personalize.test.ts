...
describe('personalizeText', () => {
  ...
  it('replaces {me} with the reader first name', () => {
    expect(
      personalizeText('What could {me} do to make mornings easier?', names)
    ).toBe('What could Adam do to make mornings easier?');
  });
  ...
  it('capitalizes a fallback at the start of the text', () => {
    expect(
      personalizeText('{partner} had a hard day. What helped?', { partnerName: null, selfName: null })
