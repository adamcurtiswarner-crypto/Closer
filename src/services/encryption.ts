...
const KEY_PREFIX = 'stoke_couple_key_';

/**
 * Generate a random 256-bit encryption key, store it locally,
 * and return the hex string. Called by the invite creator (User A).
 * The key is also written to the couple doc by useCreateInvite.
 */
export async function generateCoupleKey(coupleId: string): Promise<string> {
  // Check if we already have a key locally
...
/**
 * Fetch the couple's encryption key from the couple doc in Firestore,
 * store it locally, and return it. Called by the invite accepter (User B).
 */
export async function fetchCoupleKey(coupleId: string): Promise<string> {
  // Check local first
  const existing = await getCoupleKey(coupleId);
  if (existing) return existing;

  // Read from couple doc
  const coupleDoc = await getDoc(doc(db, 'couples', coupleId));
  const key = coupleDoc.data()?.couple_key;

  if (!key) {
    throw new Error('No encryption key found for this couple');
  }
...
/**
 * Retrieve the couple's encryption key from secure local storage.
 * Falls back to Firestore if not found locally (e.g., after reinstall).
 */
export async function getCoupleKey(coupleId: string): Promise<string | null> {
  try {
    const local = await SecureStore.getItemAsync(`${KEY_PREFIX}${coupleId}`);
...
export function encrypt(text: string, key: string): string {
  if (!key || key === 'disabled') return text;
...
export function decrypt(encoded: string, key: string): string {
  if (!key || key === 'disabled') return encoded;
  ...
  try {
    ...
    // Decryption failed — return original (may be plaintext or legacy data)
    return encoded;
  }
}
