...
**OPEN — one founder question.** Masha's account holds two Expo tokens, both currently valid (the server prunes anything Expo reports `DeviceNotRegistered`, and neither has been pruned across three weeks of daily sends). Either she has two devices — in which case two pushes is correct, one per device — or one token is a survivor of a token rotation the app missed while backgrounded, and she is getting doubles on one phone. Cannot be determined from the data; **ask her whether she has Stoke on a second device.** If not, drop the older token (`ln_cXgGSca…`, first in insertion order) and keep `A2eIq1O0…`.
